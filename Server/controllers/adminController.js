...
export const loginAdmin = async (req, res) => {
  try {
    ...
    // Hardcoded login for testing - bypass database
    if (email === 'admin@admin.com' && password === 'admin123') {
      const token = jwt.sign(
        { id: '507f1f77bcf86cd799439011', email: 'admin@admin.com', role: 'admin' },
        JWT_SECRET,
        { expiresIn: "1d" }
      );
    ...
    return res.status(401).json({ message: "Invalid credentials. Use admin@admin.com / admin123" });
  } catch (error) {
